...
<section class="top-page">
    <div class="top-inner">
        ...
        <div class="filter-button">
            <a href="/tours" class="btn btn-radial" ><?php echo pll_e('All tours');?></a>
            <?php foreach ($terms as $term) :
                    $active_class = ($term->term_id === $current_term_id) ? 'active' : '';
                ?>
                <a href="<?php echo esc_url(get_term_link($term)); ?>" class="btn btn-radial <?php echo esc_attr($active_class); ?>"><?php echo esc_html($term->name); ?></a>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
    </div>
                            ...
                            <?php if ($infoList = carbon_get_post_meta($id, 'list')) : ?>
                                ...
                                <?php  
                                    ...
                                    foreach ($infoDates as $item) :
                                        ...
                                        echo '<li class="date-item' . $hiddenClass . '"><span>Tour dates:</span> <span>' . $start_date->format('Y-m-d') . ' - ' . $end_date->format('Y-m-d') . '</span></li>';
                                ...
                                <?php foreach ($infoList as $item) : ?>
                                    <li><span><?php echo esc_html($item['text_start']); ?></span><span><?php echo esc_html($item['text_end']); ?></span></li>
                                <?php endforeach; ?>
                                </ul>
                            <?php endif; ?>
                            <div class="item-bottom">
                                <span class="item-price__info">From</span>
                                <span class="item-price"><?php echo esc_html(carbon_get_post_meta($id, 'price')); ?></span>
                                <?php $new_price_old = carbon_get_post_meta($id, 'price_old'); 
                                if ($new_price_old) { ?>
                                    <span class="item-price__old"><?php echo esc_html($new_price_old); ?></span>
                                <?php } ?>
                                <a href="<?php echo esc_url(get_the_permalink()); ?>" class="item-book btn btn-red"><?php echo pll_e('View');?></a>
                            </div>
                        </div>
                    </div>
                    <?php
